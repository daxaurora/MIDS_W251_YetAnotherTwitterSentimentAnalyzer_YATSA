...
class Tweet(dict):
    def __init__(self, tweet_raw_json, encoding='utf-8'):
        ...
        self['text'] = tweet_raw_json['text'].encode(encoding)
        self['user_id'] = tweet_raw_json['user']['id']
        self['hashtags'] = [x['text'].encode(encoding) for x in tweet_raw_json['entities']['hashtags']]
        self['timestamp'] = dateutil.parser.parse(tweet_raw_json[u'created_at']).replace(tzinfo=None).isoformat()
        self['screen_name'] = tweet_raw_json['user']['screen_name'].encode(encoding)
